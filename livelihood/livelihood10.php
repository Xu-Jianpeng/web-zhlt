...
$sql = "select * from publish where title='择一城终老，你会不会选择珠海'; ";
$query = $con->query($sql);
...
<body>
    ...
    <br>
    <hr>
    <div class="content"><p><?php echo $rows->content ?></p></div>
    <br>
    <hr>
    <div class="comment">
    <?php 
    ...
    <hr/> 
        ...
        <b style="font-size:18px"><? echo $rows2[$i];?>（用户）：</b><small style="float:right"><?echo $rows4[$i]?></small>
            ...
            <small style="float:right"><input type="submit" value="删除"  onclick="javascript:return confirm('您确认要删除评论吗？');"/></small>
            <?php }?>
            <br>
            <hr/>
        </div>
