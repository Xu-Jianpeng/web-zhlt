...
$sql = "select * from publish where title='市市场监督管理部门精准施策推动防疫与复工两手抓、两不误'; ";
$query = $con->query($sql);
...
<head>
    <meta charset="utf-8" />
    <title>市市场监督管理部门精准施策推动防疫与复工两手抓、两不误</title>
...
<body>
    ...
    <? }else {?>
        <p style="background-color:lightgrey;color:white;font-family: 宋体;font-size:30px;width:800px;height:200px;text-align:center;line-height:200px">请先登录再评论</p>
    <? } ?>    
    </table>   
    <hr/> 
        <?php 
        ...
        <b style="font-size:18px"><? echo $rows2[$i];?>（用户）：</b><small style="float:right"><?echo $rows4[$i]?></small>
            <br>
            <br><p style="margin-left: 50px;font-size:20px"><? echo $rows3[$i]?></p><br>
            <hr/>
        </div>
        <?php
        }
